 ...
 *      LEDS: F0, F4, N0, N1
 *      BOTONES: J0, J1
 * Cuando se pulsa (y se suelta)un botan, cambia de estado,
 * entre los definidos en la matriz LED. El primer botan incrementa el estado
 * y el segundo lo decrementa. Al llegar al final, se satura.
 *
 * Variante del EJ2, pero manejando los pines por interrupcian.
 * Se ha aaadido ademas el modo de bajo consumo, de manera que la espera
 * a la pulsacian de los botones se hace en modo sleep.
 ************************************************************/
...
/*********************************************************
 * Rutina de interrupcian del puerto J.
 * Leo los pines, y en cada caso hago la funcian necesaria,
 * (incrementar o decrementar el estado)
 * Establezco 20ms de debouncing en cada caso
 * Y debo borrar el flag de interrupcian al final.
 ********************************************************/
...
/*****
 * Funcian para encender los leds usando la codificacian que aparece en la matriz LED
...
int main(void)
{
    ...
    //Habilitar los perifaricos implicados: GPIOF, J, N
    ...
    SysCtlPeripheralClockGating(true);                      //Habilitar el apagado selectivo de perifaricos
    GPIOIntEnable(GPIO_PORTJ_BASE, GPIO_PIN_0|GPIO_PIN_1);  //Habilitar pines de interrupcian J0, J1
    GPIOIntRegister(GPIO_PORTJ_BASE, rutina_interrupcion);  //Registrar (definir) la rutina de interrupcian
    IntEnable(INT_GPIOJ);                                   //Habilitar interrupcian del pto J
