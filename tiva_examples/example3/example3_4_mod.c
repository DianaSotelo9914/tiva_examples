 ...
 * Si se pulsa un boton gira en un sentido, y si se pulsa el otro boton gira en sentido contrario.
 * Si no se pulsa ninguno, permanece en reposo.
 * Se introducen pasos intermedios para ganar precision
 *************************************/
...
void IntTimer(void)
{
    ...
    if(paso!=0){
    secuencia+=paso;
    if(secuencia==-1) secuencia=7;
    if(secuencia==8) secuencia=0;
